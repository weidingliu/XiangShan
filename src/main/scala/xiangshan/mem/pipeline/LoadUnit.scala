  ...
  with HasCircularQueuePtrHelper
{
  val io = IO(new Bundle() {
    ...
    val prefetch_train_l1 = ValidIO(new LdPrefetchTrainBundle())
  ...
  io.prefetch_train_l1.bits.fromLsPipelineBundle(s2_out)
  io.prefetch_train_l1.valid := s2_valid && !s2_out.mmio
  if (env.FPGAPlatform)
  ...
  val s3_valid        = RegNext(s2_valid && !s2_out.isHWPrefetch) && !RegNext(s2_out.uop.robIdx.needFlush(io.redirect))
  ...
  s3_out.valid                := s3_valid && !s3_in.isHWPrefetch && !io.lsq.ldin.bits.rep_info.need_rep && !s3_in.mmio && !s3_in.lateKill
  ...
  s3_fast_rep := (RegNext(s2_fast_rep) ||
                    ...
                    !s3_rep_frm_fetch &&
                    !s3_exception
  val s3_fb_no_waiting = !s3_in.isLoadReplay && !(s3_fast_rep && io.fast_rep_out.ready) && !s3_in.feedbacked && !s3_in.isHWPrefetch
  ...
  io.lsTopdownInfo.s2.cache_miss_en := s2_valid && s2_out.hasROBEntry && !s2_out.tlbMiss && !s2_out.missDbUpdated
