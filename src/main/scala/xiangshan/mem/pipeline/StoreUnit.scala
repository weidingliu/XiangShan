...
class StoreUnit(implicit p: Parameters) extends XSModule with HasDCacheParameters {
  ...
  val use_flow_fromRS = io.stin.valid
  val use_flow_fromPrefetch = !use_flow_fromRS && io.prefetch_req.valid
  s0_valid := use_flow_fromRS || use_flow_fromPrefetch

  val prefetch_vaddr = io.prefetch_req.bits.vaddr
  io.tlb.req.valid             := s0_valid
  io.tlb.req.bits.vaddr        := Mux(use_flow_fromRS, s0_saddr, prefetch_vaddr)
  io.tlb.req.bits.cmd          := TlbCmd.write
  io.tlb.req.bits.size         := Mux(use_flow_fromRS, LSUOpType.size(io.stin.bits.uop.ctrl.fuOpType), 3.U)
  io.tlb.req.bits.kill         := DontCare
  io.tlb.req.bits.memidx.is_ld := false.B
  io.tlb.req.bits.memidx.is_st := true.B
  io.tlb.req.bits.memidx.idx   := Mux(use_flow_fromRS, io.stin.bits.uop.sqIdx.value, DontCare)
  io.tlb.req.bits.debug.robIdx := Mux(use_flow_fromRS, io.stin.bits.uop.robIdx, DontCare)
  io.tlb.req.bits.no_translate := false.B
  io.tlb.req.bits.debug.pc     := Mux(use_flow_fromRS, io.stin.bits.uop.cf.pc, DontCare)
  io.tlb.req.bits.debug.isFirstIssue := Mux(use_flow_fromRS, io.isFirstIssue, false.B)
  io.tlb.req_kill              := false.B

  io.prefetch_req.ready := s0_can_go && io.dcache.req.fire && !io.stin.valid
  ...
  io.dcache.req.valid           := use_flow_fromRS || use_flow_fromPrefetch
  io.dcache.req.bits.cmd        := MemoryOpConstants.M_PFW
  io.dcache.req.bits.vaddr      := Mux(use_flow_fromRS, s0_saddr, prefetch_vaddr)
  io.dcache.req.bits.instrtype  := Mux(use_flow_fromRS, STORE_SOURCE.U, DCACHE_PREFETCH_SOURCE.U)

  s0_out              := DontCare
  s0_out.vaddr        := Mux(use_flow_fromRS, s0_saddr, prefetch_vaddr)
  ...
  s0_out.uop          := Mux(use_flow_fromRS, io.stin.bits.uop, DontCare)
  s0_out.miss         := DontCare
  s0_out.rsIdx        := Mux(use_flow_fromRS, io.rsIdx, DontCare)
  s0_out.mask         := Mux(use_flow_fromRS, genWmask(s0_out.vaddr, io.stin.bits.uop.ctrl.fuOpType(1,0)), 3.U)
  s0_out.isFirstIssue := Mux(use_flow_fromRS, io.isFirstIssue, false.B)
  s0_out.wlineflag    := Mux(use_flow_fromRS, io.stin.bits.uop.ctrl.fuOpType === LSUOpType.cbo_zero, false.B)
  s0_out.isHWPrefetch := use_flow_fromPrefetch
  ...
  s0_out.uop.cf.exceptionVec(storeAddrMisaligned) := Mux(use_flow_fromRS, !s0_addr_aligned, false.B)
  ...
  io.feedback_slow.valid           := s1_fire && !s1_in.isHWPrefetch
  ...
  if(EnableStorePrefetchSMS) {
    io.prefetch_train.valid := s2_valid && io.dcache.resp.fire && !s2_out.mmio && !s2_out.tlbMiss && !s2_out.isHWPrefetch
  }else {
  ...
  io.dcache.s1_paddr := s1_out.paddr
  ...
  io.dcache.s2_kill := (s2_mmio || s2_exception)
  ...
  XSPerfAccumulate("tlb_miss_first_issue",    s1_fire && s1_tlb_miss && s1_in.isFirstIssue)
    XSPerfAccumulate("sta_prefetch_num", s1_valid &&  s1_in.isHWPrefetch)
  XSPerfAccumulate("normal_store_num", s1_valid && !s1_in.isHWPrefetch)
  // end
}