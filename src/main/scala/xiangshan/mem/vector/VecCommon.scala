...
*/
object GenPackMask{
  def leadX(mask: Seq[Bool], len: Int, mod: Boolean): Bool = {
    if(len == 1){
      if (mod) {
        mask.head
      }
      ...
      if(mod){
        leadX(mask.drop(1),len-1,mod) & mask.head
      }
      else {
        leadX(mask.drop(1),len-1,mod) & !mask.head
      }
  ...
  def leadZeroVec(shiftMask: Seq[Bool]): UInt = {
    // max is 64-bit, so the max num of flow to pack is 8
  ...
  def apply(shiftMask: UInt) = {
    // pack mask
    val activePackMask = leadOneVec(shiftMask.asBools)
    val inactivePackMask = leadZeroVec(shiftMask.asBools)
...
object GenPackVec extends VLSUConstants{
  def apply(addr: UInt, shiftMask: UInt, eew: UInt): (UInt, UInt) = {
    val addrAlignedVec = CheckAligned(addr)
    val (activePackMask, inactivePackMask) = GenPackMask(shiftMask)
    // generate packVec
    val activePackVec   = addrAlignedVec & (activePackMask.asUInt >> eew)
    val inactivePackVec = addrAlignedVec & (inactivePackMask.asUInt >> eew)
...
object GenPackAlignedType extends VLSUConstants{
  def apply(packVec: UInt): UInt = {
    ...
    Mux(packVec === 0.U, "b00".U, packAlignedType) // if packVec === 0.U, packAlignedType is 0.U
  }
}
