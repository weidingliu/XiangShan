...
class VlUopQueue(implicit p: Parameters) extends VLSUModule
{
  ...
  when (io.loadRegIn.fire && !flushEnq) {
    ...
    val srcMask = GenFlowMask(Mux(vm, Fill(VLEN, 1.U(1.W)), io.loadRegIn.bits.src_mask), vvstart, evl)
  ...
  flowSplitIdx.zip(io.flowIssue).zipWithIndex.foreach { case ((flowIdx, issuePort), portIdx) =>
    ...
    val shiftMask = (issueFlowMask >> elemIdxInsideVd(portIdx)).asUInt
    val (activePackVec, inactivePackVec) = GenPackVec(vaddr, shiftMask, issueEew(1, 0))
    val activePackAlignedType = GenPackAlignedType(activePackVec.asUInt)
    // val inactivePackAlignedType = GenPackAlignedType(inactivePackVec.asUInt)
    // val packAlignedType = activePackAlignedType | inactivePackAlignedType // if packVec is zero, it's PackAlignedType is 0.U
    val packAlignedType = activePackAlignedType
    val isPackage = isUnitStride(issueInstType) && !issueEntry.fof && (packAlignedType > issueAlignedType) // don't pack fof's flow
    val packageNum = GenPackNum(issueAlignedType, packAlignedType)
